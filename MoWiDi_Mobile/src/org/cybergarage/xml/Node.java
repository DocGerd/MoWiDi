/******************************************************************
 *
 *	CyberXML for Java
 *
 *	Copyright (C) Satoshi Konno 2002
 *
 *	File: Element.java
 *
 *	Revision;
 *
 *	11/27/02
 *		- first revision.
 *	11/01/03
 *		- Terje Bakken
 *		- fixed missing escaping of reserved XML characters
 *	11/19/04
 *		- Theo Beisch <theo.beisch@gmx.de>
 *		- Added "&" and "\"" "\\" to toXMLString().
 *	11/19/04
 *		- Theo Beisch <theo.beisch@gmx.de>
 *		- Changed XML::output() to use short notation when the tag value is null.
 *	12/02/04
 *		- Brian Owens <brian@b-owens.com>
 *		- Fixed toXMLString() to convert from "'" to "&apos;" instead of "\".
 *	11/07/05
 *		- Changed toString() to return as utf-8 string.
 *	02/08/08
 *		- Added addValue().
 *
 ******************************************************************/
package org.cybergarage.xml;

import java.io.ByteArrayOutputStream;
import java.io.PrintWriter;
import java.io.UnsupportedEncodingException;
import java.util.Iterator;

public class Node {

    /**
     * Create a Node with empty UserData and no Parent Node.
     */
    public Node() {
        setUserData(null);
        setParentNode(null);
    }

    public Node(final String name) {
        this();
        setName(name);
    }

    public Node(final String ns, final String name) {
        this();
        setName(ns, name);
    }
    ////////////////////////////////////////////////
    //	parent node
    ////////////////////////////////////////////////
    private Node parentNode = null;

    public void setParentNode(final Node node) {
        parentNode = node;
    }

    public Node getParentNode() {
        return parentNode;
    }

    ////////////////////////////////////////////////
    //	root node
    ////////////////////////////////////////////////
    public Node getRootNode() {
        Node rootNode = null;
        Node parentNode = getParentNode();
        while (parentNode != null) {
            rootNode = parentNode;
            parentNode = rootNode.getParentNode();
        }
        return rootNode;
    }
    ////////////////////////////////////////////////
    //	name
    ////////////////////////////////////////////////
    private String name = new String();

    public void setName(final String name) {
        this.name = name;
    }

    public void setName(final String ns, final String name) {
        this.name = ns + ":" + name;
    }

    public String getName() {
        return name;
    }

    public boolean isName(final String value) {
        return name.equals(value);
    }
    ////////////////////////////////////////////////
    //	value
    ////////////////////////////////////////////////
    private String value = new String();

    public void setValue(final String value) {
        this.value = value;
    }

    public void setValue(final int value) {
        setValue(Integer.toString(value));
    }

    public void addValue(final String value) {
        if (this.value == null) {
            this.value = value;
            return;
        }
        if (value != null) {
            this.value += value;
        }
    }

    public String getValue() {
        return value;
    }
    ////////////////////////////////////////////////
    //	Attribute (Basic)
    ////////////////////////////////////////////////
    private AttributeList attrList = new AttributeList();

    public int getNAttributes() {
        return attrList.size();
    }

    public Attribute getAttribute(final int index) {
        return attrList.getAttribute(index);
    }

    public Attribute getAttribute(final String name) {
        return attrList.getAttribute(name);
    }

    public void addAttribute(final Attribute attr) {
        attrList.add(attr);
    }

    public void insertAttributeAt(final Attribute attr, final int index) {
        attrList.insertElementAt(attr, index);
    }

    public void addAttribute(final String name, final String value) {
        Attribute attr = new Attribute(name, value);
        addAttribute(attr);
    }

    public boolean removeAttribute(final Attribute attr) {
        return attrList.remove(attr);
    }

    public boolean removeAttribute(final String name) {
        return removeAttribute(getAttribute(name));
    }

    public boolean hasAttributes() {
        if (0 < getNAttributes()) {
            return true;
        }
        return false;
    }

    ////////////////////////////////////////////////
    //	Attribute (Extention)
    ////////////////////////////////////////////////
    public void setAttribute(final String name, final String value) {
        Attribute attr = getAttribute(name);
        if (attr != null) {
            attr.setValue(value);
            return;
        }
        attr = new Attribute(name, value);
        addAttribute(attr);
    }

    public void setAttribute(final String name, final int value) {
        setAttribute(name, Integer.toString(value));
    }

    public String getAttributeValue(final String name) {
        Attribute attr = getAttribute(name);
        if (attr != null) {
            return attr.getValue();
        }
        return "";
    }

    public int getAttributeIntegerValue(final String name) {
        String val = getAttributeValue(name);
        try {
            return Integer.parseInt(val);
        } catch (Exception e) {
        }
        return 0;
    }

    ////////////////////////////////////////////////
    //	Attribute (xmlns)
    ////////////////////////////////////////////////
    public void setNameSpace(final String ns, final String value) {
        setAttribute("xmlns:" + ns, value);
    }
    ////////////////////////////////////////////////
    //	Child node
    ////////////////////////////////////////////////
    private NodeList nodeList = new NodeList();

    public int getNNodes() {
        return nodeList.size();
    }

    public Node getNode(final int index) {
        return nodeList.getNode(index);
    }

    public Node getNode(final String name) {
        return nodeList.getNode(name);
    }

    public Node getNodeEndsWith(final String name) {
        return nodeList.getEndsWith(name);
    }

    public void addNode(final Node node) {
        node.setParentNode(this);
        nodeList.add(node);
    }

    public void insertNode(final Node node, final int index) {
        node.setParentNode(this);
        nodeList.insertElementAt(node, index);
    }

    public int getIndex(final String name) {
        int index = -1;
        for (Iterator<Node> i = nodeList.iterator(); i.hasNext();) {
            index++;
            Node n = i.next();
            if (n.getName().equals(name)) {
                return index;
            }
        }
        return index;
    }

    public boolean removeNode(final Node node) {
        node.setParentNode(null);
        return nodeList.remove(node);
    }

    public boolean removeNode(final String name) {
        return nodeList.remove(getNode(name));
    }

    public void removeAllNodes() {
        nodeList.clear();
    }

    public boolean hasNodes() {
        if (0 < getNNodes()) {
            return true;
        }
        return false;
    }

    ////////////////////////////////////////////////
    //	Element (Child Node)
    ////////////////////////////////////////////////
    public void setNode(final String name, final String value) {
        Node node = getNode(name);
        if (node != null) {
            node.setValue(value);
            return;
        }
        node = new Node(name);
        node.setValue(value);
        addNode(node);
    }

    public String getNodeValue(final String name) {
        Node node = getNode(name);
        if (node != null) {
            return node.getValue();
        }
        return "";
    }
    ////////////////////////////////////////////////
    //	userData
    ////////////////////////////////////////////////
    private Object userData = null;

    public void setUserData(final Object data) {
        userData = data;
    }

    public Object getUserData() {
        return userData;
    }

    ////////////////////////////////////////////////
    //	toString
    ////////////////////////////////////////////////
    /**
     * Inovoke {@link #getIndentLevelString(int, String)} with <code>"   "</code> as String
     *
     * @see #getIndentLevelString(int, String)
     */
    public String getIndentLevelString(final int nIndentLevel) {
        return getIndentLevelString(nIndentLevel, "   ");
    }

    /**
     *
     * @param nIndentLevel the level of indentation to produce
     * @param space the String to use for the intendation
     * @since 1.8.0
     * @return an indentation String
     */
    public String getIndentLevelString(final int nIndentLevel, final String space) {
        StringBuilder indentString = new StringBuilder(nIndentLevel * space.length());
        for (int n = 0; n < nIndentLevel; n++) {
            indentString.append(space);
        }
        return indentString.toString();
    }

    public void outputAttributes(final PrintWriter ps) {
        int nAttributes = getNAttributes();
        for (int n = 0; n < nAttributes; n++) {
            Attribute attr = getAttribute(n);
            ps.print(" " + attr.getName() + "=\"" + XML.escapeXMLChars(attr.getValue()) + "\"");
        }
    }

    public void output(final PrintWriter ps, final int indentLevel, final boolean hasChildNode) {
        String indentString = getIndentLevelString(indentLevel);

        String name = getName();
        String value = getValue();

        if (hasNodes() == false || hasChildNode == false) {
            ps.print(indentString + "<" + name);
            outputAttributes(ps);
            // Thnaks for Tho Beisch (11/09/04)
            if (value == null || value.length() == 0) {
                // Not using the short notation <node /> because it cause compatibility trouble
                ps.println("></" + name + ">");
            } else {
                ps.println(">" + XML.escapeXMLChars(value) + "</" + name + ">");
            }

            return;
        }

        ps.print(indentString + "<" + name);
        outputAttributes(ps);
        ps.println(">");

        int nChildNodes = getNNodes();
        for (int n = 0; n < nChildNodes; n++) {
            Node cnode = getNode(n);
            cnode.output(ps, indentLevel + 1, true);
        }

        ps.println(indentString + "</" + name + ">");
    }

    public String toString(final String enc, final boolean hasChildNode) {
        ByteArrayOutputStream byteOut = new ByteArrayOutputStream();
        PrintWriter pr = new PrintWriter(byteOut);
        output(pr, 0, hasChildNode);
        pr.flush();
        try {
            if (enc != null && 0 < enc.length()) {
                return byteOut.toString(enc);
            }
        } catch (UnsupportedEncodingException e) {
        }
        return byteOut.toString();
    }

    public String toString() {
        return toString(XML.CHARSET_UTF8, true);
    }

    public String toXMLString(final boolean hasChildNode) {
        String xmlStr = toString();
        xmlStr = xmlStr.replaceAll("<", "&lt;");
        xmlStr = xmlStr.replaceAll(">", "&gt;");
        // Thanks for Theo Beisch (11/09/04)
        xmlStr = xmlStr.replaceAll("&", "&amp;");
        xmlStr = xmlStr.replaceAll("\"", "&quot;");
        // Thanks for Brian Owens (12/02/04)
        xmlStr = xmlStr.replaceAll("'", "&apos;");
        return xmlStr;
    }

    public String toXMLString() {
        return toXMLString(true);
    }

    public void print(final boolean hasChildNode) {
        PrintWriter pr = new PrintWriter(System.out);
        output(pr, 0, hasChildNode);
        pr.flush();
    }

    public void print() {
        print(true);
    }
}
