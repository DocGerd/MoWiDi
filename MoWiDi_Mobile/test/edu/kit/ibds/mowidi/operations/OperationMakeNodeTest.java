 ...
 */
public final class OperationMakeNodeTest  extends AndroidTestCase{
	
	@SuppressWarnings("static-access")
	public void testAuthorizationType() {
		AbstractOperation test = new OperationMakeNode("");
		assertEquals(AuthorizationType.READ_AND_WRITE, test.minAuthorizationType);
	}
	
	public void testDoOperation() {
		File root = Environment.getExternalStorageDirectory();
		File file = new File(root, "makeFile");
		if (file.exists()) {
			file.delete();
		}
		AbstractOperation test = new OperationMakeNode("makeFile");
		test.doOperation();
